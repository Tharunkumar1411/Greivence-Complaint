...
const ProfilePage = (props) => {
    ...
    useEffect(() => {
        ...
        Axios.post("https://grievence-back.onrender.com/getComplaintCount",{Email:sessionStorage.getItem("mail")}).then((res) => {
        //  console.log(res)
        setBarData({...barData,hostel:res.data[0],academic:res.data[1],ragging:res.data[2],transport:res.data[3],others:res.data[4]})
        });
        
        Axios.put("https://grievence-back.onrender.com/getUserdetails",{Email:sessionStorage.getItem("mail")}).then((res) => {
    ...
    useEffect(() => {
        fetch(`https://grievence-back.onrender.com/getDetailsForChart?Email=${sessionStorage.getItem("mail")}`).then((data) => (data.json()).then((data) => {
